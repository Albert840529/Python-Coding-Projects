...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius = BALL_RADIUS, paddle_width = PADDLE_WIDTH,
        ...
        self.level = 1
        self.point_label = GLabel(f'Points: {self.collision_times}')
        self.point_label.font = '-20'
        self.window.add(self.point_label, x=0, y=self.window.height-self.point_label.height)
    ...
    def check_for_collision(self):
        for x in range(int(self.ball.x), int(self.ball.x+self.ball.width+1), self.ball.width):
            for y in range(int(self.ball.y), int(self.ball.y + self.ball.height + 1), self.ball.height):
                ...
                if point is not None and point is not self.point_label:
                    if point != self.paddle:
                        self.collision = 1
                        self.window.remove(point)
                        self.collision_times += 1
                        self.window.remove(self.point_label)
                        self.point_label = GLabel(f'Points: {self.collision_times}')
                        self.point_label.font = '-20'
                        self.window.add(self.point_label, x=0, y=self.window.height - self.point_label.height)
                    else:
